    ...
    <div className="pixel-panel flex flex-col w-96 max-h-[520px]">
      ...
      {tab === "completed" && (
        <div className="flex-1 overflow-y-auto space-y-1 min-h-0 pr-1">
          ...
          {groupByDate(completedTasks).map(([date, tasks]) => (
            <div key={date}>
              <button
                onClick={() => toggleCollapse(date)}
                className="text-[10px] text-gray-400 hover:text-gray-300 w-full text-left py-1 flex items-center gap-1"
              >
                <span>{collapsedDates.has(date) ? "▶" : "▼"}</span>
